...
namespace chess_pos_db_gui
{
    public partial class DatabaseCreationForm : Form
    {
        ...
        private void DatabaseCreationForm_Load(object sender, EventArgs e)
        {
            if (IsAppend)
            {
                string format = database.GetDatabaseSchema();
                databaseFormatComboBox.Items.Add(format);
                databaseFormatComboBox.SelectedItem = format;
            }
            else
            {
                var supported = database.GetSupportedDatabaseSchemas();
                foreach (var type in supported)
                {
                    databaseFormatComboBox.Items.Add(type);
                }

                databaseFormatComboBox.SelectedItem = database.GetDefaultDatabaseSchema();
            }
        }
        ...
        private void AddFiles(DataGridView dgv)
        {
            ...
            {
                MessageBox.Show("Please select the database schema first.");
                return;
            }
        ...
        private async void HandleCreateDatabaseRequest()
        {
            ...
            if (databaseFormatComboBox.SelectedItem == null || (string)databaseFormatComboBox.SelectedItem == "")
            {
                MessageBox.Show("You need to specify the database schema.");
                return;
            }
            ...
            JsonObject request = new JsonObject
            {
                ...
                { "schema", databaseFormatComboBox.Text },
        ...
        {
            MessageBox.Show(
                "This determines the binary schemas used for storing the database you're about to create.\n" +
                "Different schemas provide different tradeoffs between storage space required and the information they keep.\n" +
                "Currently the following database schemas exist:\n" +
                "\n" +
                "- db_alpha - An old deprecated schema. Not many advantages to using it now. Definitely don't use it if you have an HDD. May use less space than other schemes for small databases. If you really want to know more about this see chess_pos_db documentation on github. If you're not certain don't use it.\n" +
                "\n" +
